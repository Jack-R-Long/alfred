...
func createUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err = sqlTx.Exec("INSERT INTO users (username, email, password_hash) VALUES (?, ?, ?)", user.Username, user.Email , hashed_password)
	...
	case http.MethodGet:
		...
		json.NewEncoder(w).Encode(HealthResponse{
			...
			Data: struct {
				...
				Message:  "User found id: " + strconv.Itoa(id) + " with email " + email,
			},
		})
	case http.MethodPut:
		...
		result, err := database.DB.Exec("UPDATE users SET email = ?, password_hash = ? WHERE username = ?", user.Email, hashedPassword, username)
		if err != nil {
